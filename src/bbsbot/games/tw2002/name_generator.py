...
class NameGenerator:
    ...
    def generate_character_name(
        self, complexity: Literal["simple", "medium", "complex", "numbered"] = "medium", number_prefix: bool = False
    ) -> str:
        ...
        for attempt in range(max_attempts):
            if complexity == "simple" or (complexity == "medium" and attempt > 50):
                ...
                name = f"{self.rng.choice(PREFIXES)}{self.rng.choice(MIDDLE_CONNECTORS)}{self.rng.choice(SUFFIXES)}"

            elif complexity == "complex":
                # 4-part: Prefix + Middle1 + Middle2 + Suffix
                m1, m2 = self.rng.sample(MIDDLE_CONNECTORS, 2)
                name = f"{self.rng.choice(PREFIXES)}{m1}{m2}{self.rng.choice(SUFFIXES)}"
