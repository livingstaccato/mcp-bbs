...
if TYPE_CHECKING:
    ...
    from twbot.orientation import SectorKnowledge
        ...
        try:
            ...
            for name, record_data in data.get("records", {}).items():
                self._records[name] = CharacterRecord(
                    name=name,
                    created_at=record_data.get("created_at", time()),
                    died_at=record_data.get("died_at"),
                    inherited_from=record_data.get("inherited_from"),
                    inherited_to=record_data.get("inherited_to"),
                    sessions=record_data.get("sessions", 0),
                    total_profit=record_data.get("total_profit", 0),
                )
